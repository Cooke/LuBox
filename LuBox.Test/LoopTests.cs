...
namespace LuBox.Test
{
    ...
    public class LoopTests
    {
        ...
        {
            _environment.Variables.counter = 0;
            _luScriptEngine.Execute("for i=1, 10, 1 do counter = counter + 1 end", _environment);
            Assert.AreEqual(10, _environment.Variables.counter);
        }
        ...
        public void NumericForUseVariable()
        {
            ...
            Assert.AreEqual(45, _environment.Variables.counter);
        }
        ...
        {
            _environment.Variables.counter = 0;
            _environment.Variables.collection = new int[10];
        ...
        {
            _environment.Variables.sum = 0;
            _environment.Variables.collection = new[] { 3, 3, 3};
            _luScriptEngine.Execute("for i=0, collection.Length-1, 1 do sum = sum + collection[i] end", _environment);
            Assert.AreEqual(9, _environment.Variables.sum);
        }
        ...
        public void ForIn()
        {
            ...
            Assert.AreEqual(3, _environment.Variables.sum);
        }
        ...
        {
            _environment.Variables.sum = 0;
            _environment.Variables.collection = new [] { 3, 4, 5};
            _luScriptEngine.Execute("for i in iter(collection) do sum = sum + i end", _environment);
            Assert.AreEqual(12, _environment.Variables.sum);
        }
    }
}
